...
VN.reg = function (x, y,
            order=max(2,ceiling(VN.dep.reg(x,y)[2]*ceiling(log10(length(x))))),
            point.est = NULL,
            location = 'top',
            print.values = FALSE,
  ...
  if(length(x[x<min.range])>0){
    if(VN.dep.reg(x,y)[2]<.5){
      x0 = Dynamic.average.min} else {
        x0 = y[x==min(x)]} }  else {x0 = y[x==min(x)]}

  if(length(x[x>max.range])>0){
    if(VN.dep.reg(x,y)[2]<.5){x.max = Dynamic.average.max} else {x.max = y[x==max(x)]}}  else { x.max = y[x==max(x)]}
 ...
 ### Fitted Values
  ...
  plot(x,y,xlim=c(xmin,xmax),ylim=c(ymin,ymax),col='steelblue', xlab = "X",ylab="Y",main=paste0("Order = ",order))
